...
export default class MembersController extends Controller {
  ...
  get listHeader() {
    ...
    let count =
      members.length === 1
        ? `${members.length}${this.intl.t('Manual.Members._member')}`
        : `${members.length}${this.intl.t('Manual.Members._members')}`;

    if (selectedLabel && selectedLabel.slug) {
      members.length === 1
        ? `${count} ${this.intl.t('matches_current_filter')}`
        : `${count} ${this.intl.t('match_current_filter')}`;
    }
