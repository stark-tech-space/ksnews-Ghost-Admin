...
export default class EditorController extends Controller {
  ...
  get isOneWord() {
    if (this.get('wordCount') === null) return false;
    return this.get('wordCount').wordCount === 1;
  }
  ...
  *_savePostTask(options = {}) {
    ...
    try {
      ...
      if (isServerUnreachableError(error)) {
        const [prevStatus, newStatus] = this.post.changedAttributes().status || [
          this.post.status,
          this.post.status,
        ];
  ...
  *backgroundLoaderTask() {
  ...
  async _showScheduledNotification(delayed) {
    ...
    if (newsletter) {
      ...
      description.push(
        `${!emailOnly ? 'and delivered ' : ''}to <span><strong>${recipientCount}</strong></span>`,
      );
    }
