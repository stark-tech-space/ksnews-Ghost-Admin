...
const userValidator = BaseValidator.extend(PasswordValidatorMixin, {
  ...
  passwordChange(model) {
    ...
    if (isBlank(newPassword) && isBlank(ne2Password)) {
      model.get('errors').add('newPassword', "Sorry,_passwords_can't_be_blank");
      this.invalidate();
    } else {
      if (!validator.equals(newPassword, ne2Password || '')) {
        model.get('errors').add('ne2Password', 'Your_new_passwords_do_not_match');
        this.invalidate();
      }
  ...
  ownPasswordChange(model) {
    ...
    if (isBlank(oldPassword)) {
      model.get('errors').add('password', 'Your_current_password_is_required_to_set_a_new_one');
      this.invalidate();
    }
  },
});
