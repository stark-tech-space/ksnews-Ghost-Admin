...
export default Mixin.create({
  ...
  passwordValidation(model, password, errorTarget) {
    ...
    if (!validator.isLength(password || '', 10)) {
      model.get('errors').add(errorTarget, 'Password_must_be_at_least_10_characters_long');
    ...
    BAD_PASSWORDS.map((badPassword) => {
      if (badPassword === password) {
        model.get('errors').add(errorTarget, 'Sorry,_you_cannot_use_an_insecure_password');
        this.invalidate();
      }
    });
