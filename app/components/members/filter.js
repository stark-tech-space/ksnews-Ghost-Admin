...
const FILTER_PROPERTIES = [
  ...
  { label: 'Newsletter_subscription', name: 'subscribed', group: 'Basic' },
  { label: 'Last_seen', name: 'last_seen_at', group: 'Basic', valueType: 'date' },
  { label: 'Created', name: 'created_at', group: 'Basic', valueType: 'date' },

  // Member subscription
  { label: 'Membership_tier', name: 'tier', group: 'Subscription', valueType: 'array' },
  { label: 'Member_status', name: 'status', group: 'Subscription' },
  { label: 'Billing_period', name: 'subscriptions.plan_interval', group: 'Subscription' },
  { label: 'Stripe_subscription_status', name: 'subscriptions.status', group: 'Subscription' },
  {
    label: 'Paid_start_date',
  ...
  // Emails
  { label: 'Emails_sent_(all_time)', name: 'email_count', group: 'Email' },
  { label: 'Emails_opened_(all_time)', name: 'email_opened_count', group: 'Email' },
  { label: 'Open_rate_(all_time)', name: 'email_open_rate', group: 'Email' },
...
const CONTAINS_RELATION_OPTIONS = [
  ...
  { label: 'does_not_contain', name: 'does-not-contain' },
  { label: 'starts_with', name: 'starts-with' },
  { label: 'ends_with', name: 'ends-with' },
];
...
export default class MembersFilter extends Component {
  ...
  @tracked filters = new TrackedArray([
    new Filter({
      ...
      relation: 'is',
      value: '',
      relationOptions: FILTER_RELATIONS_OPTIONS.name.map(({ label, name }) => ({
        label: this.intl.t(`Manual.JS.${label}`),
        name: name,
      })),
    }),
  ]);
  ...
  get availableFilterProperties() {
    ...
    return availableFilters.map((filter) => ({
      ...filter,
      label: this.intl.t(`Manual.JS.${filter.label}`),
      group: this.intl.t(`Manual.JS.${filter.group}`),
    }));
  }
