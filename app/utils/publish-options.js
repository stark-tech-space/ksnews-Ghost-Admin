...
export default class PublishOptions {
  ...
  toggleScheduled(shouldSchedule) {
    ...
    if (
      shouldSchedule &&
      (!this.scheduledAtUTC || this.scheduledAtUTC.isBefore(this.minScheduledAt))
    ) {
    ...
    return [
      {
        ...
        label: 'Publish_and_email', // shown in expanded options
  ...
  get emailDisabledInSettings() {
    return (
      get(this.settings, 'editorDefaultEmailRecipients') === 'disabled' ||
  ...
  get mailgunIsConfigured() {
    return get(this.settings, 'mailgunIsConfigured') || get(this.config, 'mailgunIsConfigured');
  }
  ...
  get recipientFilter() {
    return this.selectedRecipientFilter === undefined
  ...
  *fetchRequiredDataTask() {
    ...
    // newsletters
    promises.push(
      this.store.query('newsletter', { status: 'active', limit: 'all', include: 'count.members' }),
    );
